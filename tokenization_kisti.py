...
class FullTokenizer(object):
  ...
  def __init__(self, vocab_file, do_lower_case=False, tokenizer_type="Mecab"):
    self.vocab = load_vocab(vocab_file)
    self.inv_vocab = {v: k for k, v in self.vocab.items()}

    # Basic Tokenier for Korean (Default : MeCab, other case : white space split)
    self.basic_tokenizer = KoreanTokenizer(tokenizer_type=tokenizer_type)
...
# TODO : Adding Korean Tokenizer
class KoreanTokenizer(object):
  def __init__(self, tokenizer_type="Mecab"):
    """Constructs a KoreanTokenizer.

    Args:
      tokenizer_type: set the korean tokenizer type
    """
  ...
  def tokenize(self, text):
    ...
    if self.tokenizer_type == "Mecab":
      ...
      split_tokens = mecab.morphs(text)
    else:
